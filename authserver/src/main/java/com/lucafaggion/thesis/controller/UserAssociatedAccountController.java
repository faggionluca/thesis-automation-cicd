...
public class UserAssociatedAccountController {
  ...
  public void callbackGitHub(@RequestParam String code, Authentication authentication) {
    // TODO: astrarre tutto il codice per poter essere riutilizzato da diffenti servizi, bitcucket github etc!!
    ...
    GitHubTokenExchange exGitHubTokenExchange = GitHubTokenExchange.builder().client_id(githubClientId)
        .client_secret(githubClientSecret).code(code).build();
    // Creaiamo la richiesta
    HttpEntity<GitHubTokenExchange> request = new HttpEntity<GitHubTokenExchange>(exGitHubTokenExchange, headers);
    // eseguiamo la richesta
    ResponseEntity<GitHubTokenResponse> response = restTemplate.exchange(githubAccessTokenUri, HttpMethod.POST, request,
        GitHubTokenResponse.class);
    if (response.getStatusCode() == HttpStatus.OK) {
      GitHubTokenResponse tokenResponse = response.getBody();

      // TODO: rimodulare UserAssociatedAccount con sottoclassi GithubAssociatedAccount, BitbucketAssociatedAccount etc.
      // TODO: recuperare i dettagli dell'account e salvarli insieme al token

      CustomUserDetails userDetails = (CustomUserDetails) authentication.getPrincipal();
      User user = userRepository.findByUsername(userDetails.getUsername()).orElseThrow();
      ExternalService service = externalServiceRepository.findByName(githubServiceName).orElseThrow();
